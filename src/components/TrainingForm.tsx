...
export default function TrainingForm({
  ...
  async function load() {
    ...
    const { data: facilitatorsData, error: facilitatorsError } =
      await supabase
        .from('facilitators')
    ...
    if (facilitatorsError) {
      setError('No fue posible cargar los relatores/facilitadores.');
    }

    setFacilitators((facilitatorsData ?? []) as CatalogItem[]);
  ...
  async function submit(e: FormEvent) {
    ...
    if (!form.facilitator_id) {
      setError('Selecciona un relator/facilitador.');
      return;
    }
    ...
    if (file) {
      ...
      if (uploadError) {
        setError(
          `No fue posible subir el respaldo: ${uploadError.message}`
        );
        setSaving(false);
        return;
      }
    ...
    const payload = {
      ...
      participants_count: form.participants_count
        ? Number(form.participants_count)
        : null,
    ...
    if (
      uploadedNewPath &&
      existingAttachment &&
      existingAttachment !== uploadedNewPath
    ) {
      await supabase.storage
        .from('training-evidence')
        .remove([existingAttachment]);
    }
      ...
      <div className="page-heading compact">
        <div>
          <span className="eyebrow">
            {recordId ? 'EDICIÓN' : 'NUEVO REGISTRO'}
          </span>
          ...
          <p>
            Los campos con asterisco son obligatorios. La duración se
      ...
      <form onSubmit={submit} className="form-grid">
        ...
        <Field label="FECHA" required>
          <input
            ...
            onChange={(e) =>
              setField('training_date', e.target.value)
            }
            required
          />
        </Field>
        ...
        <Field label="RELATOR / FACILITADOR" required>
          <select
            ...
            onChange={(e) =>
              setField('facilitator_id', e.target.value)
            }
            required
          >
        ...
        >
          <input
            ...
            onChange={(e) =>
              setField('activity_name', e.target.value)
            }
        ...
        <Field label="N° DE PARTICIPANTES">
          <input
            ...
            onChange={(e) =>
              setField('participants_count', e.target.value)
            }
        ...
        <Field label="OBSERVACIONES">
          <textarea
            ...
            onChange={(e) =>
              setField('observations', e.target.value)
            }
            placeholder="Opcional"
          />
        </Field>
        ...
        <Field label="RESPALDO (PDF / JPG / PNG)">
          ...
          {existingAttachment && !file && (
            <small className="helper">
              Este registro ya tiene un respaldo. Si cargas otro,
              será reemplazado.
            </small>
          )}
